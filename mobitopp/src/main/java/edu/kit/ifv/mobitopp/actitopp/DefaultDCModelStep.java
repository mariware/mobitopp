 ...
 * @author Tim Hilgert
 *
 * Objekt fr einen Discrete Choice Modell Schritt
 */
...
//TODO Methoden checken und kommentieren!
    ...
    public DefaultDCModelStep(String id, Coordinator modelCoordinator, AttributeLookup attributeLookup)
    {
        ...
        ModelFlowLists mf = modelCoordinator.getFileBase().getModelFlowLists(id); 	    	
  	    ...
  	    /*
  	     *  Parameter fr den Schritt festlegen
  	     * 	Enthlt alle Parameter die in diesem Schritt verwendet werden inkl. zugehriger Referenzen (default, person, tour, ...)
  	     */
    ...
    /**
     * 
     * Methode zur Durchfhrung des DC-Model-Schritts
     * 
     */
    @Override
    public int doStep()
    {
        ...
        // Nutzen fr jede der Alternativen bestimmen
    ...
    /**
		...
		public void initializeAlternatives(Map<String, String> inParamMap, List<String> alternativeNames)
		{
			...
			/*
			 *  Parameter laden fr jede Alternative
			 *  
			 *  mappedParameters = enthlt fr jede Alternative eine Liste (ArrayList) mit ModelParameterWeights
			 *  ModelParameterWeights enthalten Name, Gewicht (Nutzenanteil) sowie Attribut (trifft zu oder nicht bzw. Gre)
			 */
			Map<String, List<ModelParameterWeight>> mappedParameters = parameterLoader.getWeightValues(id);
		  
			/*
			 *  Attribute zu den Parametern fr jede Alternative laden
			 *  
			 *  mappedAttributes = enthlt fr jede Alternative eine Map mit Namen des Attributes und der Ausprgung fr die Alternative
			 */
    ...
    /**
		...
		private void calculateAlternatives(List<String> alternativeNames, Map<String, List<ModelParameterWeight>> mappedParameters, Map<String, Map<String, Double>> mappedAttributes)
		{
		  for (int i = 0; i < alternativeNames.size(); i++)
		  {
		    ...
		    // Referenz auf Parameter fr diese Alternative setzen
		    List<ModelParameterWeight> parameter = mappedParameters.get(alternativeNames.get(i));
		    
		    // Referenz auf Attribute fr diese Alternative
    ...
    private Map<String, Map<String, Double>> initMappedAttributes(Map<String, String> inParamMap, List<String> alternativeNames)
    {
      ...
      {
      	// Erzeuge eine eigene Map fr die Alternative
      	Map<String, Double> alternativeMap = new HashMap<String, Double>();

      	// Setzte den Grundnutzen immer aktiv, d.h. das zugehrige Attribut ist 1
      	alternativeMap.put("Grundnutzen", 1.0);
      	
      	// Ermittle die Werte fr die restlichen Attribute
        ...
        // Fge Alternativen Map ger Gesamt-Attributs-Map hinzu
