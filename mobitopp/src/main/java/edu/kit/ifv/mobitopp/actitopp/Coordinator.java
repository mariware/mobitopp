 ...
 * @author Tim Hilgert
 * 
 * Coordinator-Klasse, die die Erstellung der Wochen-Aktivittenplne koordiniert.
...
public class Coordinator
{
    ...
    // Konstante fr die Nutzung der Zeitverteilungen der Aktivittsdauern
    private static final int INDICATOR_ACT_DURATIONS = 0;
    // Konstante fr die Nutzung der Zeitverteilungen der Startzeiten
  ...
  public void executeModel() throws FileNotFoundException, IOException, InvalidPatternException
  {
  	...
  	// Durchfhrung der Modellschritte
    ...
    */			
    ...
    // Finalisierung der Wochenaktivittenplne 

  	// 1) Erstelle Startzeiten fr jede Aktivitt 
    createStartTimesforActivities();
    
    // 2) Erstelle eine Liste mit allen Aktivitten der Woche
    ...
    // 3) Erzeuge Zuhause-Aktivitten zwischen den Touren
    createHomeActivities(allModeledActivities);
    
    // 4) Wandel die Aktivittenzwecke des Modells zurck in mobiTopp-Aktivittenzwecke
   ...
   */
    ...
    // Ergebnis zu Personen-Map hinzufgen
  ...
  private void executeStep2(String id)
  {
    ...
    for (HDay currentDay : pattern.getDays())
    {
      ...
      if (activityType!='H')
      {	
        // Fge die Tour in das Pattern ein
        HTour mainTour = new HTour(currentDay, 0);
        currentDay.addTour(mainTour);
                  
        // Fge die Aktivitt in das Pattern ein
   ...
   */
    ...
    for (HDay currentDay : pattern.getDays())
    {   	
    	// Ist der Tag durch Home bestimmt, wird der Schritt nicht ausgefhrt
      ...
      // Erstelle die weiteren Touren an diesem Tag basierend auf der Entscheidung und fge Sie in das Pattern ein
    ...
    for (HDay currentDay : pattern.getDays())
    {
      ...
      for (HTour currentTour : currentDay.getTours())
      {
        ...
        if (currentTour.getIndex() != 0)
        {
          ...
          // Speichere gewhlte Entscheidung fr weitere Verwendung
          char chosenActivityType = step.getAlternativeChosen().charAt(0);

          //Erstelle fr den gewhlten Tourtyp die Hauptaktivitt der Tour
   ...
   */
    ...
    for (HDay currentDay : pattern.getDays())
    {
      ...
      for (HTour currentTour : currentDay.getTours())
      {
  	    ...
  	    // Erstelle die weiteren Aktivitten in dieser Tour basierend auf der Entscheidung und fge Sie in das Pattern ein
        for (int j = 1; j <= step.getDecision(); j++)
        {
        	HActivity act = null;
        	// 3A - Touren vor der Haupttour
	...
	{
		// Wird nur ausgefhrt, wenn es zu dem Aktivittentyp auch Aktivitten gibt
	  if (pattern.countActivitiesPerWeek(activitytype)>0)
	  {
	    ...
	    // Ergebnis als Index und Alternative zu Personen-Map hinzufgen fr sptere Verwendung
    ...
    for (HDay currentDay : pattern.getDays())
    {
      ...
      for (HTour currentTour : currentDay.getTours())
      {
      	// Anwendung des Modellschritts nur auf Hauptaktivitten
	...
	{
		// Modifizierte Zeitverteilungen zur Modellierung von hheren Auswahlwahrscheinlichkeiten bereits gewhlter Zeiten
    modifiedActDurationDTDs = new DiscreteTimeDistribution[Configuration.NUMBER_OF_ACTIVITY_TYPES][Configuration.NUMBER_OF_ACT_DURATION_CLASSES];
    for (HDay currentDay : pattern.getDays())
    {
      if (currentDay.isHomeDay())
	    ...
	    if (person.isPersonWorkerAndWorkMainToursAreScheduled())
	    {
	      for (HDay currentDay : pattern.getDays())
	      {
		    	...
		    	if (tourtype == 'W' || tourtype == 'E')
		      {
		        ...
		        // Bestimme Ober- und Untergrenze und schrnke Alternativenmenge ein
		  	    modifyAlternativesForStep10B(currentDay, currentTour, step);
		  	    
		  	    // Fhre Entscheidungswahl durch
		  	    step.doStep();
		...
		for (HDay currentDay : pattern.getDays())
    {
      ...
      HTour currentTour = currentDay.getTour(0);
    	...
    	// Fhre Schritt nur fr Haupttouren aus, die noch keine festgelegte Startzeit haben
   ...
   */
		...
		for (HDay currentDay : pattern.getDays())
    {
      ...
      HTour currentTour = currentDay.getTour(currentDay.getLowestTourIndex()+(tournrdestages-1));
    	...
    	// Fhre Schritt nur fr Touren aus, die noch keine festgelegte Startzeit haben
    	if (!currentTour.isScheduled())
      {
    		// AttributeLookup erzeugen
    		AttributeLookup lookup = new AttributeLookup(person, currentDay, currentTour);   	
	...
	/**
	 * 
	 * Bestimmung des genaueren Aktivittenzwecks fr den Zweck Shopping 'S', 'L' und 'W'
	 * 
	 * @param activity
	 * @param id
	 */
	...
	{
	  // STEP 98A-C Verfeinerung Aktivittenzweck SHOPPING, LEISURE und WORK
	...
	{
//TODO Optimierungspotential - die Aktivitt darf nicht lnger sein als die noch brige Zeit - 
//TODO nicht nur abhngig von den Hauptaktivitten sondern auch den restlichen bisher festgelegten Aktivitten
	    
		// Obergrenze 1 - bisher festgelegte "verbrauchte" Zeiten am Tag
			int totalMainActivityTime = 0;
			// Alle Hauptaktivitten + zugehrige Wege + letzter Weg am Ende der Tour
		...
		// Obergrenze 2 - Aktivitt muss krzer sein als Hauptaktivitt auf Tour 
		
		    // Tim (08.11.2016) - Obergrenze default auf 99999 Minuten gesetzt, das heit ohne Wirkung, da sonst zu kurze Aktivitten)
	...
	private int[] calculateBoundsForHomeTime(HDay day, HTour tour, boolean categories) throws InvalidPatternException
  {
    ...
    // Gehe alle verbleibenden Touren des Tages durch und bercksichtige bereits feststehende Dauern fr die Festlegung der Grenzen
	...
	{
		// Bestimme die Grenzen auf Basis bereits festgelegter Aktivittszeiten
	  int bounds[] = calculateStartingBoundsForMainTours(day, true);
	  int lowerbound = bounds[0];
	  int upperbound = bounds[1];
	
	  // Prfe, ob die Tour im Standard-Startzeitraum liegt
	  ...
	  // Schrnke die Alternativen entsprechend der Grenzen ein
	...
	/**
   * 
   * Bestimmt die Ober- und Untergrenze der Startzeiten fr Touren
   * Boolean-Wert categories bestimmt, ob die Zeitkategorien oder die konkreten Grenzwerte zurckgegeben werden
   * 
   * @param categories
   * @param day
   * @param tour
  ...
  {
  	// lowerbound startet mit 1 - upperbound mit 1440 (0 Uhr nchster Tag)
    int lowerbound = 1;
    int upperbound = 1440;
    ...
    // Gehe alle Touren des Tages durch und bercksichtige bereits feststehende Dauern fr die Festlegung der Grenzen
    // +1 um jeweils nach der Tour noch eine Heimaktivitt von min. einer Minute zu ermglichen
    for (int i = tour.getIndex(); i <= day.getHighestTourIndex(); i++)
    {
    ...
    // Zeitklassen fr erste Tour des Tages
    if(categories && tour.getIndex()== day.getLowestTourIndex())
    {
      // Setze die Zeiten in Kategorien um
	    ...
	    // Zeitklassen fr zweite und dritte Tour des Tages
    if(categories && tour.getIndex()!= day.getLowestTourIndex())
    {
      // Setze die Zeiten in Kategorien um
	...
	/**
   * 
   * Bestimmt die Ober- und Untergrenze der Startzeiten fr Haupttouren
   * Boolean-Wert categories bestimmt, ob die Zeitkategorien oder die konkreten Grenzwerte zurckgegeben werden
   * 
   * @param categories
   * @param day
  ...
  {
  	// lowerbound startet mit 0 - upperbound mit 1619 (2h59 nachts nchster Tag)
    int lowerbound = 0;
    int upperbound = 1619;
    ...
    // Auf die untere Grenze kommt noch die Zeit fr den Startweg der aktuellen Tour dazu
    lowerbound += Configuration.FIXED_TRIP_TIME_ESTIMATOR;
    
    
    // Die obere Grenze wird auf maximal 23.59 Uhr nachts gesetzt fr den Startzeitpunkt der Tour
	...
	/**
	 * 
	 * Bestimmt die Ober- und Untergrenze der Startzeiten fr Touren vor der Haupttour
	 * Boolean-Wert categories bestimmt, ob die Zeitkategorien oder die konkreten Grenzwerte zurckgegeben werden
	 * 
	 * @param day
	 * @param tour
...
// TODO if Bedingung rausnehmen - Tour sollte scheduled sein in diesem Schritt
	...
	/**
	 * 
	 * Bestimmt die Ober- und Untergrenze der Startzeiten fr Touren nach der Haupttour
	 * Boolean-Wert categories bestimmt, ob die Zeitkategorien oder die konkreten Grenzwerte zurckgegeben werden
	 * 
	 * @param day
	 * @param tour
...
//TODO Checken, ob korrekte Methode aufgerufen wird - ActivityTime oder Tourtime
  ...
  /**
   * 
   * Festlegung von Default-Wegzeiten fr alle Aktivitten
   * 
   */
  private void createTripTimesforActivities() 
  {
	...
	/**
   * 
   * Erstellt Startzeiten fr jede Aktivitt
   * 
   */
  private void createStartTimesforActivities()
  {
      for (HDay day : pattern.getDays())
      {
          ...
          for (HTour tour : day.getTours())
          {
              ...
              for (HActivity act : tour.getActivities())
              {
              	// Bei erster Aktivitt in Tour wird die Startzeit durch den Beginn der Tour bestimmt
              	if (act.isActivityFirstinTour())
              	{
              		act.setStartTime(tour.getStartTime() + act.getEstimatedTripTime());
              	}
              	// Ansonsten durch das Ende der vorherigen Aktivitt
              	else
              	{
  ...
  /**
   * 
   * Methode erzeugt Home-Aktivitten zwischen den Touren
   * 
   */
  private void createHomeActivities(List<HActivity> allmodeledActivities)
  {
  	...
  	{	
    	// Erstelle zunchst eine Home-Aktivitt vor Beginn der ersten Tour
    	int duration1 = allmodeledActivities.get(0).getTripStartTimeWeekContext();
//TODO ndern
    	// Es muss immer eine H-Akt zu Beginn mglich sein
    	assert duration1>0 : "Fehler - keine Home-Aktivitt zu Beginn mglich!";
    	if (duration1>0)
    	{
    		pattern.addHomeActivity(new HActivity(pattern.getDay(0), homeact, duration1, 0));
    	}
    	
    	// Durchlaufe alle Aktivitten in der Woche - bis auf die letzte
    	for (int i=0; i<allmodeledActivities.size()-1; i++)
    	{
    		HActivity act = allmodeledActivities.get(i);
    		// Wenn Aktivitt die letzte der Tour ist, erzeuge Heimaktivitt
    		if (act.isActivityLastinTour())
    		{
    			...
    			assert duration2>0 : "Fehler - keine Home-Aktivitt nach Ende der Tour mglich!";
    			// Bestimme zugehrigen Tag zu der Heimaktivitt
    			int day = (int) ende_tour/1440;
    			// Fge Heimaktivitt in Liste hinzu
    			if (duration2>0)
    			{
    	...
    	// Prfe, ob nach der letzten Aktivitt noch Zeit fr Heim-Aktivitt ist
  	...
  	// In diesem Fall ist die Aktivittenliste komplett leer - erzeuge eine Heimaktivitt fr die ganze Woche
    else
    {
   ...
   */
		...
		for (HActivity act : allActivities_inclHome)
		{
			switch (act.getType())
			{
				...
				case 'H':
					...
					break;
				default:
					System.err.println("Ungltiger Modellaktivittentyp");
			}
		}
	}
	...
	/**
	 * 
	 * Methode zum berprfen fr berlappende Aktivitten
	 * 
	 * @param weekpattern
	 * @return
	 * @throws InvalidPatternException
